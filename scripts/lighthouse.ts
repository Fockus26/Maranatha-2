...
const ROUTES = (process.env.ROUTES ?? "/,/proyectos,/proyectos/techo-para-el-salon-multiusos,/dashboard,/dashboard/proyectos").split(",");
...
for (const route of ROUTES) {
  ...
  writeFileSync(`${OUT_DIR}/${slug}.html`, Array.isArray(report) ? report[0] : report);
  writeFileSync(`${OUT_DIR}/${slug}.json`, Array.isArray(report) ? report[1] : JSON.stringify(lhr, null, 2));
  ...
  for (const cat of CATEGORIES) {
    ...
    const problems = category.auditRefs
      ...
      .filter((a) => a && a.score !== null && a.score < 1 && a.scoreDisplayMode !== "informative" && a.scoreDisplayMode !== "manual" && a.scoreDisplayMode !== "notApplicable");

    if (problems.length === 0) continue;
    // El dashboard es `noindex` a propósito: su score de SEO no cuenta.
    const isPrivate = route.trim().startsWith("/dashboard");
    if ((cat === "accessibility" || (cat === "seo" && !isPrivate)) && (category.score ?? 0) < 1) failed = true;

    console.log(`  ${cat}:`);
    for (const a of problems) {
      const items = a.details && "items" in a.details ? (a.details.items as unknown[]).length : 0;
      console.log(`    • ${a.title}${items ? ` (${items})` : ""}`);
      const desc = a.description.replace(/\s*\[.*?\]\(.*?\)/g, "").replace(/\n/g, " ").trim();
